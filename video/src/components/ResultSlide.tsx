      ...
      <div style={{ display: "flex", width: "100%", height: "100%", padding: "40px 64px", gap: 36, zIndex: 1 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ opacity: headerOpacity }}>
            <div style={{ fontSize: 24, color: colors.accent, fontWeight: 600, letterSpacing: 4 }}>FINAL REPORT</div>
            <h2 style={{ fontSize: 48, fontWeight: 800, margin: "8px 0 0" }}>Root Cause 분석 완료</h2>
          </div>
          ...
          <div style={{ display: "flex", gap: 14, flex: 1 }}>
            <div style={{ flex: 1, borderRadius: 16, overflow: "hidden", position: "relative" }}>
              ...
              <div style={{ position: "absolute", top: 12, left: 12, backgroundColor: `${colors.primary}dd`, borderRadius: 8, padding: "8px 18px", fontSize: 24, fontWeight: 700, color: "#fff" }}>TEMPLATE (Clean)</div>
            </div>
            ...
            <div style={{ flex: 1, borderRadius: 16, overflow: "hidden", position: "relative" }}>
              ...
              <div style={{ position: "absolute", top: 12, left: 12, backgroundColor: `${colors.accent}dd`, borderRadius: 8, padding: "8px 18px", fontSize: 24, fontWeight: 700, color: "#fff" }}>TESTED (Defect!)</div>
              {boundingBoxOpacity > 0.5 && (
                <div style={{ position: "absolute", bottom: 14, left: 14, right: 14, backgroundColor: `${colors.bg}ee`, borderRadius: 10, padding: "12px 18px", fontSize: 24, fontWeight: 700, color: colors.accent, fontFamily: "'JetBrains Mono', monospace", textAlign: "center" }}>
                  Defect @ (x:120, y:45) | Type: Micro-Short
                </div>
              )}
            </div>
          </div>
          <div style={{ fontSize: 22, color: colors.textMuted, textAlign: "center" }}>Source: DeepPCB Dataset - PCB Defect Detection Benchmark</div>
        </div>
        ...
        <div style={{ width: 540, display: "flex", flexDirection: "column", gap: 24, flexShrink: 0, justifyContent: "center" }}>
          <div style={{ opacity: causeOpacity, borderLeft: `5px solid ${colors.accent}`, paddingLeft: 28 }}>
            <div style={{ fontSize: 26, fontWeight: 700, color: colors.accent, letterSpacing: 2, marginBottom: 12 }}>ROOT CAUSE TRACE</div>
            <div style={{ fontSize: 26, lineHeight: 1.6, color: colors.textSecondary }}>
              <span style={{ color: colors.accent, fontWeight: 700 }}>Error 0x08</span> {"-> "}SDA-GND Short 후보
              {"-> "}CAD U3-Pin5 / Via-B2 매핑
            </div>
            <div style={{ fontSize: 28, lineHeight: 1.6, color: colors.text, marginTop: 6 }}>
              PCB 확대 검증: <span style={{ color: colors.accent, fontWeight: 700 }}>(120,45) 미세 단락</span> 검출
            </div>
            <div style={{ fontSize: 26, lineHeight: 1.6, color: colors.text, marginTop: 6 }}>
          ...
          <div style={{ opacity: causeOpacity, borderLeft: `5px solid ${colors.warning}`, paddingLeft: 28 }}>
            <div style={{ fontSize: 26, fontWeight: 700, color: colors.warning, letterSpacing: 2, marginBottom: 12 }}>IMPACT</div>
            <div style={{ fontSize: 28, lineHeight: 1.6, color: colors.text }}>I2C 통신 마비 {"-> "}센서 데이터 수집 불가</div>
            <div style={{ fontSize: 28, lineHeight: 1.6, color: colors.text, marginTop: 6 }}>
          ...
          <div style={{ opacity: actionOpacity, borderLeft: `5px solid ${colors.primary}`, paddingLeft: 28 }}>
            <div style={{ fontSize: 26, fontWeight: 700, color: colors.primary, letterSpacing: 2, marginBottom: 12 }}>SPEC-BASED RESOLUTION</div>
            <div style={{ fontSize: 26, lineHeight: 1.6, color: colors.textSecondary, marginBottom: 4 }}>
              IPC-2221C Sec 6.1: 최소 이격 0.1mm (설계 0.15mm = PASS)
            </div>
            <div style={{ fontSize: 28, lineHeight: 1.6, color: colors.text }}>
