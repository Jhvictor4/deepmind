    ...
    <AbsoluteFill style={fullScreen}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 36, zIndex: 1, padding: "48px 72px", width: "100%" }}>
        <div style={{ opacity: headerOpacity, textAlign: "center" }}>
          <div style={{ fontSize: 24, color: colors.accent, fontWeight: 600, letterSpacing: 4, marginBottom: 14 }}>THE PROBLEM</div>
          <h2 style={{ fontSize: 56, fontWeight: 800, margin: 0, lineHeight: 1.25 }}>
            ...
            return (
              <div key={i} style={{ opacity: o, transform: `translateY(${interpolate(s, [0, 1], [50, 0])}px)`, flex: 1, maxWidth: 420, display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ padding: "0 4px" }}>
                  <div style={{ fontSize: 32, fontWeight: 700, color: colors.accent, marginBottom: 6 }}>{p.label}</div>
                  <div style={{ fontSize: 24, color: colors.textSecondary, lineHeight: 1.4 }}>{p.desc}</div>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ opacity: interpolate(frame, [70, 90], [0, 1], { extrapolateRight: "clamp" }), fontSize: 28, color: colors.textSecondary, textAlign: "center", lineHeight: 1.5 }}>
          ...
          <span style={{ fontSize: 22, color: colors.textMuted }}> (최신 DRAM 검증 기준)</span>
        </div>
      </div>
    </AbsoluteFill>
  );
